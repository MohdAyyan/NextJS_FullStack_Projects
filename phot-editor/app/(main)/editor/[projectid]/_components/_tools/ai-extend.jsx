...
// Helper: Load fabric image with Promise
const loadImage = (url) => {
...
function AIExtenderControls({ project }) {
  ...
  const applyExtension = async () => {
    ...
    if (!mainImage || !selectedDirection || !project) return;
    ...
    try {
      ...
      const extendedImage = await loadImage(extendedUrl);
